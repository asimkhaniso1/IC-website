...
import { BarChart3, ClipboardList, Factory, FileCheck2, History, Home, LogOut, PackageOpen, Settings2, ShoppingCart, Truck } from 'lucide-react';
import { useSession } from '../auth/useSession';
import { COMPANY } from '../lib/constants';

export default function FactoryShell({ children }: { children: ReactNode }) {
  const { session, signOut } = useSession();
  const nav = [{ to: '/factory', label: 'Home', icon: Home }, { to: '/factory/production', label: 'Production', icon: Factory }, { to: '/factory/grn', label: 'Goods receiving', icon: Truck }, { to: '/factory/purchase-requests', label: 'Purchase requests', icon: ShoppingCart }, { to: '/factory/gate-pass', label: 'Gate pass', icon: FileCheck2 }, { to: '/factory/stock', label: 'Stock ledger', icon: PackageOpen }, { to: '/factory/stock/register', label: 'Stock register', icon: ClipboardList }, { to: '/factory/reports', label: 'Reports', icon: BarChart3 }, { to: '/factory/audit', label: 'Audit log', icon: History }, { to: '/factory/setup', label: 'Setup', icon: Settings2 }];
