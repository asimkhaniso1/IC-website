...
export async function listDispatchData(){const s=client();const[goods,passes,dispatches]=await Promise.all([s.from('finished_goods').select('finished_goods_id,quantity,status,product:products(product_code,description),uom:uoms(uom_code),batch:production_batches(batch_code)').eq('status','AVAILABLE'),s.from('gate_passes').select('gate_pass_id,serial_no,company_name,vehicle_no').eq('status','ISSUED').is('dispatch_id',null),s.from('dispatches').select('dispatch_id,dispatch_no,dispatch_date,quantity,vehicle_no,driver_name,status,customer:customers(customer_name),goods:finished_goods(product:products(product_code,description)),uom:uoms(uom_code)').order('created_at',{ascending:false})]);for(const r of[goods,passes,dispatches])if(r.error)throw r.error;return{goods:goods.data??[],passes:passes.data??[],dispatches:dispatches.data??[]}}
...
export async function listTraceability(){const{data,error}=await client().from('production_batches').select('production_batch_id,batch_code,status,actual_quantity,machine:machines(machine_name),order:production_orders(production_order_no,status,order_item:order_items(order:orders(order_no,customer:customers(customer_name)),product:products(product_code,description)),entries:production_entries(production_entry_id,report_date,total_meter,operator:operators(full_name)),inspections:qc_inspections(qc_inspection_id,inspection_date,overall_result,defects:qc_defects(defect_type,defect_severity)),packing:packing_records(packing_record_no,quantity,roll_count,carton_count),finished:finished_goods(quantity,status),stock:stock_ledger(stock_ledger_id,transaction_date,quantity,stock_category,source_transaction_type)').order('created_at',{ascending:false});if(error)throw error;return data??[]}
