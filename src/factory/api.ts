...
export async function listQuotationData(){const s=client();const[sources,customers,products,quotes,costings,lines]=await Promise.all([s.rpc('list_factory_quote_sources_v2'),s.from('customers').select('customer_id,customer_code,customer_name').eq('active_status',true).order('customer_name'),s.from('products').select('product_id,product_code,description').eq('active_status',true).order('description'),s.from('factory_quotations').select('*,customer:customers(customer_code,customer_name),product:products(product_code,description)').order('created_at',{ascending:false}),s.from('quote_costings').select('*').eq('status','FINAL'),s.from('quote_costing_lines').select('*')]);for(const r of[sources,customers,products,quotes,costings,lines])if(r.error)throw r.error;return{sources:(sources.data??[])as any[],customers:customers.data??[],products:products.data??[],quotes:quotes.data??[],costings:(costings.data??[]).map((c:any)=>({...c,lines:(lines.data??[]).filter((l:any)=>l.quote_costing_id===c.quote_costing_id)}))}}
...
export async function listSampleDevelopmentData(){const s=client();const[sources,samples]=await Promise.all([s.rpc('list_factory_sample_sources_v2'),s.from('factory_samples').select('*').order('created_at',{ascending:false})]);for(const r of[sources,samples])if(r.error)throw r.error;return{sources:(sources.data??[])as any[],samples:samples.data??[]}}
export async function createFactorySample(rfq:string,assignedTo:string,targetDate:string,notes:string){const{data,error}=await client().rpc('create_factory_sample_v2',{p_rfq:rfq,p_assigned_to:assignedTo,p_target_date:targetDate||null,p_notes:notes||null});if(error)throw error;return data}
...
export async function listSampleCostingData(){const s=client();const[samples,costings,lines]=await Promise.all([s.from('factory_samples').select('*').order('created_at',{ascending:false}),s.from('sample_costings').select('*'),s.from('sample_costing_lines').select('*').order('line_no')]);for(const r of[samples,costings,lines])if(r.error)throw r.error;return{samples:samples.data??[],costings:(costings.data??[]).map((c:any)=>({...c,lines:(lines.data??[]).filter((l:any)=>l.sample_costing_id===c.sample_costing_id)}))}}
export async function saveSampleCosting(input:{sample:string;currency:string;overhead:number;status:string;notes:string;lines:unknown[]}){const{data,error}=await client().rpc('save_sample_costing',{p_sample:input.sample,p_currency:input.currency,p_overhead_percent:input.overhead,p_status:input.status,p_notes:input.notes||null,p_lines:input.lines});if(error)throw error;return data}
export async function listQuotePreparation(){const s=client();const[sources,specs,costings,lines]=await Promise.all([s.rpc('list_factory_quote_preparation_sources_v2'),s.from('production_specs').select('*'),s.from('quote_costings').select('*'),s.from('quote_costing_lines').select('*').order('line_no')]);for(const r of[sources,specs,costings,lines])if(r.error)throw r.error;return{sources:sources.data??[],specs:specs.data??[],costings:(costings.data??[]).map((c:any)=>({...c,lines:(lines.data??[]).filter((l:any)=>l.quote_costing_id===c.quote_costing_id)}))}}
export async function saveQuotePreparation(input:{rfq:string;details:unknown;currency:string;overhead:number;status:string;sampleRequired:boolean;notes:string;lines:unknown[]}){const{data,error}=await client().rpc('save_quote_preparation_v2',{p_rfq:input.rfq,p_details:input.details,p_currency:input.currency,p_overhead_percent:input.overhead,p_status:input.status,p_sample_required:input.sampleRequired,p_notes:input.notes||null,p_lines:input.lines});if(error)throw error;return data}
...
export async function listQuickBooksSync(){const s=client();const[data,jobs]=await Promise.all([listQuickBooksExportData(),s.from('quickbooks_sync_queue').select('*').order('created_at',{ascending:false}).limit(100)]);if(jobs.error)throw jobs.error;return{...data,jobs:jobs.data??[]}}
export async function queueQuickBooksSync(kind:string,ids:string[]){const{data,error}=await client().rpc('queue_quickbooks_sync',{p_entity_type:kind,p_record_ids:ids});if(error)throw error;return data}
